...
function client_mt:__init(args)
	...
	for key, value in pairs(args) do
		...
		elseif key == "driver" then
			assert(value_type == "table", "expecting driver to be a table")
			a.st_driver = value
		elseif key == "device" then
			assert(value_type == "table", "expecting device to be a table")
			a.st_device = value
		else
	...
	assert(a.st_driver, 'expecting driver="..." to create MQTT client')
	assert(a.st_device, 'expecting device="..." to create MQTT client')
...
function client_mt:close_connection(reason)
	...
	if self.keepalive_timer then
		args.st_driver:cancel_timer(self.keepalive_timer)		-- Cancel Keep-Alive routine timer *****
		self.keepalive_timer = nil
	end
...
function client_mt:open_connection()
	...
	self.keepalive_timer = self.args.st_driver:call_on_schedule(self.args.keep_alive, 
			function()
				self:send_pingreq()
			end,
			'Keep-alive timer')
